...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_earnings'])) {
    foreach ($_POST['earnings_percentage'] as $partner_id => $percentage) {
        ...
        try {
            $stmt = $db->prepare("UPDATE senior_partners SET earning = :earning WHERE id = :id");
             $stmt->execute([
                 ':earning' => $percentage,
                 ':id' => $partner_id
             ]);
...
$senior_partners = [];
try {
    $stmt = $db->query("SELECT id, name, email, earning FROM senior_partners ORDER BY name ASC");
    $senior_partners = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $error_message = "Error fetching senior partners: " . $e->getMessage();
}
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Manage Senior Partner Earnings - Admin Dashboard</title>
        <!-- Bootstrap CSS -->
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h5>Manage Senior Partner Earnings</h5>
                        </div>
                        <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="4">No senior partners found.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
