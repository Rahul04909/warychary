...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
            ...
            if(in_array(strtolower($ext), $allowed)) {
                ...
                if(move_uploaded_file($_FILES['product_image']['tmp_name'], $upload_path . $new_filename)) {
                    ...
                    if(!empty($product['product_image'])) {
                        $old_image_path = $upload_path . $product['product_image'];
        ...
        $offer_product_image = $product['offer_product_image']; // Keep existing image by default
        if(isset($_FILES['offer_product_image']) && $_FILES['offer_product_image']['error'] == 0) {
            ...
            if(in_array(strtolower($ext), $allowed)) {
                ...
                if(move_uploaded_file($_FILES['offer_product_image']['tmp_name'], $upload_path . $new_filename)) {
                    // Delete old image if exists
                    if(!empty($product['offer_product_image'])) {
                        $old_image_path = $upload_path . $product['offer_product_image'];
                        if(file_exists($old_image_path)) {
                            unlink($old_image_path);
                        }
                    }
        ...
        $query = "UPDATE products SET 
            ...
            offer_product_purchase_price = :offer_product_purchase_price, 
            offer_product_sales_price = :offer_product_sales_price, 
            offer_product_mrp = :offer_product_mrp,
                        ...
                        <div class="form-section">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="product_name" name="product_name" required value="<?php echo htmlspecialchars($product['product_name']); ?>">
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <?php if(!empty($product['product_image'])): ?>
                                        <div class="current-image">
                                            <p>Current image:</p>
                                            <img src="../images/products/<?php echo $product['product_image']; ?>" alt="Current Product Image" class="img-thumbnail" style="max-height: 100px;">
                                        </div>
                                    <?php endif; ?>
                                    <input type="file" class="form-control" id="product_image" name="product_image" accept="image/*" onchange="previewImage(this, 'product_image_preview')">
                                    <div id="product_image_preview" class="image-preview" style="display: none;"></div>
                                    <small class="form-text text-muted">Leave empty to keep current image</small>
                                </div>
                                
                                <div class="col-md-12 mb-3">
                                    <label for="product_description" class="form-label">Product Description <span class="text-danger">*</span></label>
                                    <textarea id="product_description" name="product_description" class="form-control summernote" required><?php echo htmlspecialchars($product['product_description']); ?></textarea>
                                </div>
                                
                                <div class="col-md-4 mb-3">
                                    <label for="purchase_price" class="form-label">Purchase Price (₹) <span class="text-danger">*</span></label>
                                    <input type="number" step="0.01" class="form-control" id="purchase_price" name="purchase_price" required onchange="calculateTotal()" value="<?php echo $product['purchase_price']; ?>">
                                </div>
                                
                                <div class="col-md-4 mb-3">
                                    <label for="sales_price" class="form-label">Sales Price (₹) <span class="text-danger">*</span></label>
                                    <input type="number" step="0.01" class="form-control" id="sales_price" name="sales_price" required value="<?php echo $product['sales_price']; ?>">
                                </div>
                                
                                <div class="col-md-4 mb-3">
                                    <label for="mrp" class="form-label">MRP (₹) <span class="text-danger">*</span></label>
                                    <input type="number" step="0.01" class="form-control" id="mrp" name="mrp" required value="<?php echo $product['mrp']; ?>">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="offer_product_name" name="offer_product_name" value="<?php echo htmlspecialchars($product['offer_product_name'] ?? ''); ?>">
                                </div>
                                
                                <div class="col-md-6 mb-3">
                                    <label for="offer_product_image" class="form-label">Offer Product Image</label>
                                    <?php if(!empty($product['offer_product_image'])): ?>
                                        <div class="current-image">
                                            <p>Current image:</p>
                                            <img src="../images/products/<?php echo $product['offer_product_image']; ?>" alt="Current Offer Product Image" class="img-thumbnail" style="max-height: 100px;">
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div id="offer_product_image_preview" class="image-preview" style="display: none;"></div>
                                    <small class="form-text text-muted">Leave empty to keep current image</small>
                                </div>
                                
                                <div class="col-md-4 mb-3">
                                    <label for="offer_product_purchase_price" class="form-label">Purchase Price (₹)</label>
                                    <input type="number" step="0.01" class="form-control" id="offer_product_purchase_price" name="offer_product_purchase_price" value="<?php echo $product['offer_product_purchase_price'] ?? 0; ?>" onchange="calculateTotal()">
                                </div>
                                
                                <div class="col-md-4 mb-3">
                                    <label for="offer_product_sales_price" class="form-label">Sales Price (₹)</label>
                                    <input type="number" step="0.01" class="form-control" id="offer_product_sales_price" name="offer_product_sales_price" value="<?php echo $product['offer_product_sales_price'] ?? 0; ?>">
                                </div>
                                
                                <div class="col-md-4 mb-3">
                                    <label for="offer_product_mrp" class="form-label">MRP (₹)</label>
                                    <input type="number" step="0.01" class="form-control" id="offer_product_mrp" name="offer_product_mrp" value="<?php echo $product['offer_product_mrp'] ?? 0; ?>">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" step="0.01" class="form-control" id="delivery_cost" name="delivery_cost" required value="<?php echo $product['delivery_cost']; ?>" onchange="calculateTotal()">
                                </div>
                                
                                <div class="col-md-6 mb-3">
                                    <label for="packing_cost" class="form-label">Packing Cost (₹) <span class="text-danger">*</span></label>
                                    <input type="number" step="0.01" class="form-control" id="packing_cost" name="packing_cost" required value="<?php echo $product['packing_cost']; ?>" onchange="calculateTotal()">
                                </div>
                            </div>
                            ...
                            <div class="total-expense-box">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 text-end">
                                        <h4>Total: ₹<span id="total_expense_display"><?php echo number_format($product['total_expense'], 2); ?></span></h4>
                                        <input type="hidden" id="total_expense" name="total_expense" value="<?php echo $product['total_expense']; ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
